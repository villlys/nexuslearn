    ...
    try {
        ...
        if (!['profesor', 'alumno'].includes(rol)) {
            return res.status(400).json({
                error: "El rol debe ser 'profesor' o 'alumno'."
            });
        }
       ...
       VALUES ($1, $2, $3, $4) RETURNING id`, [nombre, email, password_hash, rol]
        );
        ...
        if (error.code === '23514') {
            return res.status(400).json({ error: 'Uno de los valores enviados no es válido (verifica el rol)' });
        }
        console.error(error);
...
router.put('/:id/avatar', verificarToken, async(req, res) => { // ← CAMBIO
    ...
    try {
        await pool.query(
            'UPDATE USUARIOS SET avatar = $1 WHERE id = $2', [avatar, id]
        );
