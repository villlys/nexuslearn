...
router.post('/', verificarToken, verificarProfesor, async(req, res) => {
    ...
    try {
       ...
       VALUES ($1, $2, $3, $4) RETURNING id`, [titulo, descripcion, id_profesor, codigo_acceso]
        );
...
router.get('/:codigo_acceso', async(req, res) => {
    ...
    try {
        const actividad = await pool.query(
            'SELECT * FROM ACTIVIDADES_GAMIFICADAS WHERE codigo_acceso = $1', [codigo_acceso]
        );
       ...
       FROM PREGUNTAS WHERE id_actividad = $1`, [actividad.rows[0].id]
        );
...
// DELETE /api/actividades/:id - El profesor elimina una de sus actividades
router.delete('/:id', verificarToken, verificarProfesor, async(req, res) => {
    const { id } = req.params;

    try {
        // Primero verificamos que la actividad exista y sea del profesor que la solicita
        const actividad = await pool.query(
            'SELECT id_profesor FROM ACTIVIDADES_GAMIFICADAS WHERE id = $1', [id]
        );
        ...
        if (actividad.rows[0].id_profesor !== req.usuario.id) {
            return res.status(403).json({ error: 'No puedes eliminar actividades de otro profesor' });
        }

        // El ON DELETE CASCADE de tu esquema borra automáticamente
        // las preguntas y el historial de respuestas relacionados
        await pool.query('DELETE FROM ACTIVIDADES_GAMIFICADAS WHERE id = $1', [id]);
